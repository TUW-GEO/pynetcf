...
class OrthoMultiTest(unittest.TestCase):
    ...
    def test_file_io_simple(self):
        ...
        with nc.OrthoMultiTs(self.testfilename, mode='w', n_loc=3) as dataset:
            for n_data in [5]:
    ...
    def test_file_io_2_steps(self):
        ...
        with nc.OrthoMultiTs(self.testfilename, n_loc=3, mode='w') as dataset:
            for n_data in [5]:
                for location in [1, 2, 3]:
        ...
        with nc.OrthoMultiTs(self.testfilename, n_loc=3, mode='a') as dataset:
            for n_data in [5]:
                for location in [1, 2, 3]:
    ...
    def test_file_write_ts_all_1_location(self):
        ...
        with nc.OrthoMultiTs(self.testfilename, n_loc=1, mode='w') as dataset:
            n_data = 5
...
class DatasetContiguousTest(unittest.TestCase):
    ...
    def test_file_writing(self):
        ...
        with nc.ContiguousRaggedTs(self.testfilename, n_loc=3,
                                   n_obs=9, mode='w') as dataset:
            ...
            dataset.write_ts(1, data, dates, loc_descr='first station',
                             lon=0, lat=0, alt=5)
            dataset.write_ts(2, data, dates, loc_descr='first station',
                             lon=0, lat=0, alt=5)
            dataset.write_ts(3, data, dates, loc_descr='first station',
                             lon=0, lat=0, alt=5)
...
class GriddedNcIndexedRaggedTsTests(unittest.TestCase):
    ...
    def test_writing_with_attributes(self):
        ...
        dataset = nc.GriddedNcIndexedRaggedTs(self.testdatapath,
                                              self.grid,
                                              mode='w')
        for gpi in [10, 11, 12]:
            dataset.write_gp(gpi, ts, attributes=attributes)
